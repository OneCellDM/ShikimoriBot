...
using BotShared.ShikimoriSharp.Exceptions;

namespace BotShared.ShikimoriSharp.Bases
{
    public abstract class ApiBase
    {
        ...
        protected ApiBase(Enums.Version version, ApiClient apiClient)
        {
            Version = version;
        ...
        private static HttpContent DeserializeToRequest<T>(T obj)
        {
            ...
            {
                it.Name,
                Value = typeooft.GetField(it.Name)?.GetValue(obj)
            })
                .Where(it => !(it.Value is null));
            var content = new MultipartFormDataContent();
            foreach (var i in typeEnum)
            {
                Debug.WriteLine(i.Name + ":" + i.Value);
